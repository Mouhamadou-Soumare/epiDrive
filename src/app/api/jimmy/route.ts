...
const findExistingRecipe = async (title: string) => {
  ...
  return closestMatch
    ? await prisma.recette.findUnique({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Vous êtes un assistant expert en analyse de recettes.",
          },
          {
            role: "user",
            content: [
              {
                type: "text",
                text: "Analyse cette image et extrait la recette complète avec ingrédients détaillés.",
              },
              {
                type: "image_url",
    ...
    const detectedRecipe = JSON.parse(
    ...
    const { ingredientsToAdd, productsToAdd } = await processIngredients(
      detectedRecipe.ingredients
    );
