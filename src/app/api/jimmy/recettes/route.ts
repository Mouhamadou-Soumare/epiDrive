...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(panier) || panier.length === 0) {
      return NextResponse.json(
        { error: "Invalid or empty panier" },
        { status: 400 }
      );
    }

    const ingredientsList: string[] = panier
      .map((item: { produit: Ingredient }) => item.produit?.name)
      .filter(Boolean);

    if (ingredientsList.length === 0) {
      return NextResponse.json(
        { error: "No valid ingredients found in panier" },
        { status: 400 }
      );
    }

    console.log(
      " Recherche de recettes avec les ingrédients :",
      ingredientsList
    );
        ...
        messages: [
          {
            role: "system",
            content:
              "Vous êtes un assistant utile pour générer des recettes. Respectez scrupuleusement le schéma donné.",
          },
          {
            role: "user",
    ...
    for (const recipe of recipes) {
      ...
      let existingRecipe = allRecipes.find(
        (r) => normalizeString(r.title) === normalizedTitle
      );
