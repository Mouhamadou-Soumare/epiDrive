// app/api/sendOrderConfirmation/route.tsx
...
export async function POST(req: Request) {
  try {
    ...
    if (!userName || !commandeId || !email) {
      throw new Error("Les informations de l'utilisateur ou de la commande sont manquantes.");
    }

    const senderEmail = process.env.RESEND_EMAIL_FROM;
    if (!senderEmail) {
      throw new Error("L'adresse email d'expédition (RESEND_EMAIL_FROM) doit être définie dans le fichier .env.");
    }
