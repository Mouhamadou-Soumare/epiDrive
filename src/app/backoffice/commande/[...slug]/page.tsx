...
import {
  useGetCommande,
  useUpdateCommande,
} from "@/hooks/commandes/useCommandes";
...
const CommandeDetail = () => {
  ...
  const {
    ...
    error: commandeError,
  } = useGetCommande(commandeSlug) as {
  ...
  const {
    ...
    loading: userLoading,
    error: userError,
  } = useGetUser(commande?.fk_userId) as {
    user: User | null;
    loading: boolean;
    error: string | null;
  };
  ...
  const produits: Produit[] =
    commande?.quantites?.map((quantite) => ({
      ...quantite.produit,
      prix: quantite.prix,
    })) || [];
  ...
  const handleUpdateStatus = async (newStatus: CommandeStatus) => {
    if (commande) {
      try {
        const updatedCommande = await updateCommande(commande.id, {
          ...commande,
          status: newStatus,
        });
        ...
        setMessageCommandeAlert(
          `Le statut de la commande ${commande.id} est maintenant "${newStatus}".`
        );
        ...
        <div className="relative overflow-hidden rounded-lg bg-white px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
          ...
          <dd className="ml-16 flex items-baseline pb-6 sm:pb-7">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gray-50 px-4 py-4 sm:px-6">
              <Link
                href={`/backoffice/utilisateur/${user.id}`}
                className="font-medium a-primary svg-hover flex flex-row items-center"
              >
          ...
          <dd className="ml-16 flex items-center pb-6 sm:pb-7">
            <div className="relative w-full">
              <select
                ...
                onChange={(e) =>
                  handleUpdateStatus(
                    e.target.value as unknown as CommandeStatus
                  )
                }
