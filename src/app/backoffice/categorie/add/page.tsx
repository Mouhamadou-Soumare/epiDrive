...
export default function AddCategoryPage() {
  ...
  const handleInputChange = (
    ...
    setCategory({
      ...category,
      [name]:
        name === "imageId" || name === "parentId" ? parseInt(value) : value,
    });
  };
  ...
  return (
    <>
      ...
      {submitResult === "500" && (
        <Alert message="Error during adding category" type="error" />
      )}
      {submitResult === "400" && (
        <Alert message="Please fill in all required fields" type="warning" />
      )}
      {addError && <Alert message="Failed to add category" type="error" />}
      {errorCategories && (
        <Alert message="Failed to load categories" type="error" />
      )}
